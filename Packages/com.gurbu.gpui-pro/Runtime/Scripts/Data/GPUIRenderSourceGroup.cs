...
//#endif
        ...
        public static int GetKey(int prototypeKey, GPUIProfile profile, int groupID, List<string> shaderKeywords)
        {
            if (shaderKeywords == null || shaderKeywords.Count == 0)
                return HashCode.Combine(prototypeKey, profile.GetInstanceID(), groupID);
            shaderKeywords.Sort();
            return HashCode.Combine(prototypeKey, profile.GetInstanceID(), groupID, string.Concat(shaderKeywords).GetHashCode());
        }

        public int GetRenderSourceKey(UnityEngine.Object source)
        {
            return HashCode.Combine(source.GetInstanceID(), Key);
        }
    ...
    public class GPUIRenderSource : IGPUIDisposable
    {
        ...
        {
            return HashCode.Combine(source.GetInstanceID(), renderSourceGroup.Key);
        }
    }
